						...
						<a href="<%=basePath %>getActivityDetail?activityId=${act.ActivityID}&activityStatus=${activityStatus}&thirdpartyAccount=${thirdpartyAccount}">
							<div class="list">
								<c:if test="${activityStatus == 1 }">
									<c:if test="${act.AttendStatus == 1 }">
						              	<div class="status status_03">申请中</div>
					              	</c:if>
					              	<c:if test="${act.AttendStatus == 2 }">
						              	<div class="status status_03">申请成功</div>
					              	</c:if>
								</c:if>
...
$(function(){
	...
	function loadActivity(){
		...
		$.ajax({
			...
			url : "${pageContext.request.contextPath}/getMoreActivityList?activityStatus=-1&pageIndex="+pageIndex+"&origin=3&thirdpartyAccount="+thirdpartyAccount,
			dataType : "json",
