			...
			<c:forEach items="${activityList }" var="act">
				<section class="">
					<a href="<%=basePath %>getActivityDetail?activityId=${act.ActivityID}&activityStatus=${activityStatus}&thirdpartyAccount=${thirdpartyAccount}">
						<div class="list">
							<%-- <c:choose>
								<c:when test="${act.IsApply && activityStatus == 1 && act.ActivityType == 1 }">
				              	</c:when>
								<c:when test="${act.IsApply && activityStatus == 1 && act.ActivityType != 1 }">
										<div class="status status_01">报名中</div>
				              	</c:when>
				              	<c:when test="${act.IsApply && activityStatus == 1 }">
				              		<div class="status status_02">已报满</div>
				              	</c:when>
							</c:choose> --%>
								
				           
							<div class="title">${fn:length(act.ActivityName)>12 ? fn:substring(act.ActivityName,0,12):act.ActivityName}${fn:length(act.ActivityName)>12 ? '...':''}</div>
   ...
   $(function(){ 
	...
	function loadActivity(){
		...
		$.ajax({
			...
			cache : false,
			url : "${pageContext.request.contextPath}/getMoreActivityList?activityStatus=1&pageIndex="+pageIndex+"&origin=1&thirdpartyAccount="+thirdpartyAccount,
			dataType : "json",
