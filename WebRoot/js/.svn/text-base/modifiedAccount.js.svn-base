...
$(function(){
		...
		$.ajax({
			...
			success:function(rs){
				if(rs.Status==200){//如果手机没找到，提示需要注册
					...
					$("#dialog2").show();
					$("#dialog2 .weui_dialog_bd").text("该手机号已被其它用户注册");			
					$(".weui_loading_toast").hide ();
	...
	$("#showTooltips").on("tap",function(event){
		...
		$.ajax({
			type:"GET",
			url:"user/modifiedAccount",
			data:"LoginID="+entireNumber+"&Captcha="+Vcode+"&OpenID="+OpenID,
			success:function(rs){
				if(rs.Status==109){
					$("#dialog2").show();
					...
					$("#dialog2").show();
					$("#dialog2 .weui_dialog_bd").text("更换手机号失败");
					$(".weui_loading_toast").hide ();
				}
			},
			error:function(){
